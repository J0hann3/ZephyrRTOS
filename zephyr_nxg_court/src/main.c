...
static const struct i2c_dt_spec lum_device = I2C_DT_SPEC_GET(I2C_LUM_NODE);
static const struct i2c_dt_spec temp_device = I2C_DT_SPEC_GET(I2C_TEMP_NODE);
static const struct gpio_dt_spec vcc_sensor = GPIO_DT_SPEC_GET(DT_ALIAS(vcc_sensor), gpios);

uint16_t read_temp_sensor(const struct i2c_dt_spec temp_device, uint16_t *temp, uint16_t *humidity)
{
...
uint16_t read_lum_sensor(const struct i2c_dt_spec lum_device, uint16_t *light)
{
